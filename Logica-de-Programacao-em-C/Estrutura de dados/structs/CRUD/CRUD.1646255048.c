...
void main(void) {
	...
	while(1) {
		...
		if(fun==0) {
			...
			//localizando cadastros inativos
			x=0;
			while(cadastro[x].ativo!=0) {
				x++;
			}
			...
			while(y!=1) {
				...
				scanf("%i",&cadastro[x].ID);
				//Verificação de range de ID escohido
				if(i>100) {
					printf("Sem espaço para novos cadastros\n\n");
					break;
					return(fun=0);
				} else
					for(int z=0; z<=i; z++) {
						if(cadastro[z].ID==cadastro[i].ID&&z!=i) {
		...
		//atualizar usuário
			...
			while(y!=1) {
				...
				scanf("%i",&IDrefresh);
				for(int x=0; cadastro[x].ID!=IDrefresh; x++) {
				}
				if(x>=100) {
